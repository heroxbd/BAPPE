...
class Zern(ABC):
    ...
    def Rnm(self, k, rho):
        ...
        """
        return jnp.polyval(self.rhotab[k, :], rho)

    def I_Rnmrho(self, k, rho):
        r"""Compute :math:`\int R_n^m(\rho)\rho`."""
        return sum(
            [(rho ** (self.n + 2 - i)) * self.rhoitab[k, i] for i in range(self.n + 3)]
        )
    ...
    def make_cart_grid(self, xx, yy, unit_circle=True):
        ...
        """
        self.ZZ = np.zeros((xx.size, self.nk), order="F", dtype=self.numpy_dtype)
        self.shape = xx.shape
    ...
    def fit_cart_grid(self, Phi, rcond=None):
        ...
        a, res, rnk, sv = lstsq(np.dot(zfA.T, zfA), np.dot(zfA.T, Phi1), rcond=rcond)
    ...
    def save(self, filename, prepend=None, params=HDF5_options, libver="latest"):
    ...
    def save_h5py(self, f, prepend=None, params=HDF5_options):
        ...
        f.create_dataset(
            prefix + "normalise", data=np.array([self.normalise], dtype=np.int)
        )
...
class RZern(Zern):
    ...
    def angular(self, j, theta):
        m = self.mtab[j]
        if m >= 0:
            return jnp.cos(m * theta)
        else:
            return jnp.sin(-m * theta)
...
class FitZern:
    ...
    def __init__(self, z, L, K):
        ...
        rho_j = np.array([np.cos(np.pi * (K - (j + 0.5)) / (2 * K)) for j in range(K)])
        rho_a = np.array([np.cos(np.pi * (K - (j + 0.0)) / (2 * K)) for j in range(K)])
        rho_b = np.array([np.cos(np.pi * (K - (j + 1.0)) / (2 * K)) for j in range(K)])
        self.rho_j = rho_j
        self.rho_a = rho_a
        self.rho_b = rho_b
                    ...
                    for j in range(K):
                        ...
                        for i in range(L):
                            av[tmp1 + i] = (
                                (1.0 / np.pi)
                                * I_Rnmrho[tmp2]
                                * (I_cosm[offL + i] + 1j * sgn * I_sinm[offL + i])
                            )
                ...
                for j in range(self.K):
                    ...
                    for i in range(self.L):
                        ak += (
                            ...
                            * Phi[tmp1 + i]
                            * self.I_Rnmrho[tmp2]
                            * (self.I_cosm[offL + i] + 1j * sgn * self.I_sinm[offL + i])
                        )
            else:
